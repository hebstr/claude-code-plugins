...
Two [Claude Code](https://claude.com/claude-code) plugins bundling slash-command skills: `audit` (review workflows) and `workflow` (project workflow automation).
Each skill is invoked explicitly via its slash command.
See each plugin's README for skill-level details.
See the [Claude Skills documentation](https://support.claude.com/en/articles/12512180-using-skills-in-claude) for how Claude Skills work.
...
The rest of the skill runs.
  ...
  | ------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
  | `OPENROUTER_API_KEY` env var                                                                                                               | [`audit:blindspot`](./audit/blindspot/), [`audit:walkthrough`](./audit/walkthrough/)                                                  | Cross-model judging and cross-provider validation via [OpenRouter](https://openrouter.ai)                                                                                                              |
  | `python3` (standard library only)                                                                                                          | [`audit:blindspot`](./audit/blindspot/), [`audit:walkthrough`](./audit/walkthrough/), [`audit:mcp-adversary`](./audit/mcp-adversary/) | Reviewer scan (`scan-reviewers.py`), the L2 OpenRouter verdict call (`openrouter-verdict.py`), the generation-ID check of every OpenRouter call (`openrouter-generation.py`), and MCP server discovery |
  | `jq` CLI                                                                                                                                   | [`audit:blindspot`](./audit/blindspot/), [`audit:walkthrough`](./audit/walkthrough/)                                                  | Required to build the cross-model judge request in `audit:blindspot`. In `audit:walkthrough` it reads the installed Ouroboros version and the PR body, and degrades when absent                        |
  | [Ouroboros](https://github.com/Q00/ouroboros) plugin                                                                                       | [`audit:walkthrough`](./audit/walkthrough/)                                                                                           | QA verdicts, evaluate + drift checks, lateral-think rescue                                                                                                                                             |
  | [`critical-code-reviewer`](https://github.com/posit-dev/skills) (posit-dev/skills)                                                         | [`audit:sweep`](./audit/sweep/), [`audit:walkthrough`](./audit/walkthrough/)                                                          | Specialist code-review agent. Default reviewer in `audit:sweep`; recommended `--reviewer` for `audit:walkthrough` on code targets. `audit:sweep` falls back to inline review when absent               |
  ...
  | `gh` CLI                                                                                                                                   | [`audit:walkthrough`](./audit/walkthrough/)                                                                                           | PR body as drift seed when no review goal is available (falls back to latest commit message)                                                                                                           |
