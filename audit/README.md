...
Review workflows for code, Claude Code skills and MCP servers.
Covers point-by-point walkthroughs, full-project sweeps, and adversarial cross-model review with circularity detection.
...
## Skills
  ...
  | --------------------------------------- | ------------------------------------------------------------------------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
  | [`walkthrough`](./walkthrough/)         | `/audit:walkthrough [target] [--reviewer name] [--batch\|--no-batch] [--revisit-deferred]` | Interactive, point-by-point walkthrough of a review report. Orchestrator mode launches a reviewer first; walkthrough-only mode processes an existing report; revisit-deferred mode processes the project's `.claude/DEFERRED.md` backlog and rewrites it in place. Re-evaluates each finding, proposes fixes, checks impacted files for regressions, waits for approval before moving on. |
  | [`sweep`](./sweep/)                     | `/audit:sweep [path]`                                                                      | Full-coverage project review. Detects project type and size, spawns specialist background agents with disjoint scopes (architecture, quality, tests, docs), consolidates findings into one deduplicated report sorted by severity, then offers an interactive walkthrough.                                                                                                                |
  | [`blindspot`](./blindspot/)             | `/audit:blindspot <target-path> [--reviewer <audit-skill>]`                                | Reviewer orchestrator that detects circularity (reviewer shares its target's codebase, prompts, or model family) and runs cross-model judging via [OpenRouter](https://openrouter.ai), then convergence analysis over the resulting verdicts.                                                                                                                                             |
...
- `sweep` is the broad-coverage entry point for whole-project audits.
...
**Optional**, per skill.
Each skill degrades gracefully: a missing dependency disables the affected feature and is reported.
The rest of the skill runs.

  | Skill                                | Optional dependency                                                                                                                  | When missing                                                                                                                                              |
  | ------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------- |
  | `walkthrough`                        | [Ouroboros](https://github.com/Q00/ouroboros) plugin                                                                                 | QA verdicts, evaluate + drift, and lateral-think rescue are skipped. Transparency block reports the missing layers.                                       |
  | `walkthrough`, `blindspot`           | [OpenRouter](https://openrouter.ai) API key (`OPENROUTER_API_KEY` env var), plus `jq` CLI for `blindspot`                            | Cross-provider validation (walkthrough) and cross-model judging (blindspot) disabled. Blindspot falls back to single-model mode with an explicit warning. |
  | `walkthrough`                        | [GitHub CLI](https://cli.github.com) (`gh`)                                                                                          | PR body cannot be fetched for the drift seed when no review goal is available. Falls back to the latest commit message.                                   |
  ...
  | `walkthrough` (`--reviewer` flag)    | [`critical-code-reviewer`](https://github.com/posit-dev/skills) (posit-dev/skills)                                                   | `--reviewer posit-dev:critical-code-reviewer` is unavailable; pick another reviewer from the scanned list.                                                |
...
`sweep`'s Agent B (Architecture & Structure) uses Claude Code's built-in `Explore` subagent.
